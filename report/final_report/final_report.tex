...
\begin{itemize}
  ...
  \item $w^{(d)}$ (\verb"word_incidences") is a dictionary containing for each distinct word (indexed by its position in the vocabulary) the number of times it appears in the document. The size of $w^{(d)}$ is $N_d \times 2$.
\end{itemize}
...
\begin{itemize}
\setlength\itemsep{-0.2em}
  \item $\alpha> 0$ is chosen randomly. %We have chosen $\alpha = 0.5$.
