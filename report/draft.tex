...
Initialize $\gamma_i^{(d)} = \alpha + \frac{N}{k}$ for all $i$ \;
\While{not converged}{
\For{$n=1\ldots N$}{
\For{$i=1\ldots k$}{
$\phi_{ni}^{(d)} = \beta_iw_n\exp(\Psi(\gamma_i^{(d)}))$
}
normalize $\phi_n^{(d)}$ to sum to $1$.
}
$\gamma^{(d)} = \alpha + \sum_{n=1}^N \phi_n^{(d)}$
}
}
\end{algorithm}
...
\KwResult{\texttt{dirich$\_$param} ($\alpha$), \texttt{word$\_$prob$\_$given$\_$topic} ($\beta$)}
\Begin{
$\beta \propto \sum_{d \in D} (\phi^{(d)})^{\top}w^{(d)}$ (which corresponds to $\beta_{ij} \propto \sum_{d \in D} \sum_{n=1}^{N_d} \phi_{ni}^{(d)} w_{nj}^{(d)}$)

\While{not converged}{
$\alpha \leftarrow \alpha - \frac{L'(\alpha)}{L''(\alpha)}$ where $\begin{cases}L'(\alpha) = |D| k \left[ \Psi\left( k \alpha \right) - \Psi(\alpha)\right] + \sum_{d \in D} \left[ \sum_{i=1}^k \Psi (\gamma_i^{(d)}) - \Psi\left( \sum_{j=1}^k \gamma_j^{(d)}\right) \right] \\
L''(\alpha) = |D|k [k\Psi'(\alpha) - \Psi' \left( \alpha\right)] \end{cases}$
}
}
\end{algorithm}
