...
def latent_dirichlet_allocation(corpus, nb_topics, voc_size,
    ...
    converged = var_inf_stop(threshold = 1e-4, max_iter = max_iter)
    ...
    while(not converged(corpus_log_likelihood)):
        ...
        (dirich_param, word_logproba_given_topic, corpus_log_likelihood) \
            = maximization_step(corpus, dirich_param, word_logproba_given_topic,
                                var_inf_max_iter = var_inf_max_iter,
                                logger = logger)
...
def variational_inference(document, dirich_param, word_logprob_given_topic,
                          verbose = False,
                          var_inf_max_iter = 20, logger = None):
    ...
    (incident_words, word_incidences, subvoc_size, nb_topics, log_var_multinom,
     var_dirich, log_likelihood, log_likelihoods, stop) = _var_inf_init(
         document,dirich_param, word_logprob_given_topic, var_inf_max_iter)
...
def _var_inf_init(
    ...
    stop = var_inf_stop(threshold = 1e-6, max_iter = var_inf_max_iter)
    log_likelihoods = []
...
def maximization_step(corpus, old_dirich, log_old_word_proba,
    ...
    # for each document and its corresponding var_dirich (gamma)
    # and var_multinom (phi)
    var_dirich_cat = []
    for (index, document) in enumerate(corpus):
        # E-step
        (var_dirich, log_var_multinom, log_likelihoods) = variational_inference(
            document, old_dirich, log_old_word_proba,
            var_inf_max_iter = var_inf_max_iter, verbose = False)
        
        if(logger): var_dirich_cat.append(var_dirich)
    ...
    while(nb_iter < 20 and (convergence_threshold < error or error < 0)):
        ...
        hessian = compute_hessian_wrt_dirich_param(dirich_param,
                                                   num_docs, nb_topics)
...
#def compute_gradient_wrt_dirich_param(dirich_param, var_dirich, num_docs,
#num_topics):
#    gradient = num_docs*num_topics*(psi(num_topics*dirich_param) \
#    - psi(dirich_param))
